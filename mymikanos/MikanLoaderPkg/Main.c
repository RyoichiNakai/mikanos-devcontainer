...
EFI_STATUS EFIAPI UefiMain(
  ...
  VOID* kernel_buffer;
  // カーネルファイルを読み込むための一時領域をバイト単位で確保する
  status = gBS->AllocatePool(
    EfiLoaderData,    // メモリの確保の仕方 
    kernel_file_size, // 確保するメモリ領域の種別
    &kernel_buffer    // 一時保管されたメモリ領域の先頭アドレスを保存
  );
  if (EFI_ERROR(status)) {
  ...
  // プログラムヘッダのLOADの部分をAllocatePageした最終目的地にコピー
  CopyLoadSegments(kernel_ehdr);
  Print(L"Kernel: 0x%0lx - 0x%0lx\n", kernel_first_addr, kernel_last_addr);

  // kernel_bufferの一時領域を解放
